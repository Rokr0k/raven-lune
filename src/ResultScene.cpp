...
void ResultScene::initialise(App *app)
{
    ...
    int w, h;
    cleared = font::renderText(app->renderer, result.cleared ? "Stage Cleared" : "Stage Failed", 50);
    SDL_QueryTexture(cleared, NULL, NULL, &w, &h);
    clearedRect = {320 - (float)w / h * 25, 30, (float)w / h * 50, 50};
    font::loadSize(30);
    maxCombo = font::renderText(app->renderer, "Max Combo: " + std::to_string(result.maxCombo), 30);
    ...
    for (int i = 0; i < 5; i++)
    {
        judge[i] = font::renderText(app->renderer, judges[i] + ": " + std::to_string(result.judgeCount[i]), 30);
    ...
    {
        rank = font::renderText(app->renderer, ranks[(result.judgeCount[0] * 2 + result.judgeCount[1]) * 9 / (result.noteCount * 2)], 160);
    }
    else
    {
        rank = font::renderText(app->renderer, "?", 160);
    }
